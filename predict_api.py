...
# Store latest reading from ESP32
latest_reading = {}

@app.route("/update", methods=["POST"])
def update():
    """ESP32 posts sensor data here — dashboard reads it back"""
    global latest_reading
    data = request.get_json()
    latest_reading = data
    latest_reading["timestamp"] = __import__("datetime").datetime.now().strftime("%H:%M:%S")
    return jsonify({"status": "saved"})

@app.route("/latest", methods=["GET"])  
def latest():
    """Dashboard polls this to get most recent ESP32 reading"""
